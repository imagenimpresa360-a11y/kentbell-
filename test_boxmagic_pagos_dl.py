...
async def evaluate_and_download():
    load_dotenv()
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # Boxmagic uses Daterangepicker. Let's fire the setStartDate / setEndDate JS event
            print("Attempting to set dates to January 2026...")
            try:
                # Based on standard daterangepicker config
                await page.evaluate('''() => {
                    // Try setting standard global daterangepicker
                    if(window.$ && $('#reportrange').length) {
                        $('#reportrange').data('daterangepicker').setStartDate('01/01/2026');
                        $('#reportrange').data('daterangepicker').setEndDate('31/01/2026');
                ...
                # If there's a button "filtrar" let's click it
                filtrar_btn = page.locator('text="filtrar"')
                if await filtrar_btn.count() > 0:
                    await filtrar_btn.first.click()
            ...
            try:
                # Usually there is a buttons-csv class
                csv_btn = page.locator('.buttons-csv, text="CSV"')
                if await csv_btn.count() > 0:
                    async with page.expect_download(timeout=30000) as dl_info:
                        await csv_btn.first.click()
                    dl = await dl_info.value
                    path = "pagos_campanario.csv"
                    await dl.save_as(path)
                    print(f"Saved successfully: {path}")
                else:
                    print("No CSV button found. Trying 'Exportar' #exportar")
                    export_btn = page.locator('#exportar')
                    if await export_btn.count() > 0:
                        async with page.expect_download(timeout=30000) as dl_info:
                            await export_btn.click()
                        dl = await dl_info.value
                        path = "pagos_campanario_export.xlsx"
                        await dl.save_as(path)
                        print(f"Saved successfully: {path}")
...
if __name__ == "__main__":
    asyncio.run(evaluate_and_download())

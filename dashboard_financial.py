...
def render_financial_dashboard(engine, start_date, end_date, sede_filter):
    ...
    try:
        ...
        # ===== TABS PRINCIPALES =====
        tab_pl, tab_cf, tab_cartola = st.tabs(["📊 P&L Dashboard", "💰 Cash Flow Statement", "🏦 Cartola + Cuenta Contable"])
        ...
        with tab_cf:
            st.markdown('''
                <div style='background-color: #f8fafc; padding: 15px; border-radius: 8px; border-left: 5px solid #10b981; margin-bottom: 20px;'>
                    <h3 style='margin-top:0;'>💰 Cash Flow Statement (Resumen de Flujo de Caja)</h3>
                    <p style='margin-bottom:0;'>Ingresos reales vs Egresos reales en el periodo seleccionado, agrupados por categoría. Ideal para auditoría rápida y entender hacia dónde va el dinero.</p>
                </div>
            ''', unsafe_allow_html=True)
            ...
            with c_cf1:
                ...
                st.metric("Total Ingresos Brutos", f"${total_income:,.0f}")
                st.metric("Costos Transaccionales (Est.)", f"-${total_commissions:,.0f}")
                st.metric("Total Ingresos Netos", f"${net_commercial:,.0f}")
                
            with c_cf2:
                st.markdown("#### 🔴 Egresos por Categoría")
                st.metric("Total Egresos (OpEx)", f"${total_expenses:,.0f}")
                if not df_cat.empty:
                    df_cat_disp = df_cat.copy()
                    df_cat_disp['total'] = df_cat_disp['total'].apply(lambda x: f"${x:,.0f}")
                    st.dataframe(df_cat_disp, use_container_width=True)
                
            st.markdown("---")
            flujo_color = '#10b981' if (net_commercial - total_expenses) >= 0 else '#ef4444'
            st.markdown(f"<h2 style='text-align: center; color: {flujo_color};'>Flujo Neto Operacional: <b>${(net_commercial - total_expenses):,.0f}</b></h2>", unsafe_allow_html=True)

        with tab_cartola:
            st.markdown('''
                <div style='background-color: #f8fafc; padding: 15px; border-radius: 8px; border-left: 5px solid #3b82f6; margin-bottom: 20px;'>
                    <h3 style='margin-top:0;'>🏦 Cartola Mensual Asignada</h3>
                    <p style='margin-bottom:0;'>Revisa los movimientos reales del banco y su cuenta contable asignada, al estilo Excel.</p>
                </div>
            ''', unsafe_allow_html=True)
            
            q_cartola = text('''
                SELECT 
                    b.bank_date as "Fecha", 
                    b.description as "Descripción Banco", 
                    b.amount as "Monto", 
                    b.balance as "Saldo",
                    COALESCE(c.name, 'Sin Asignar / Pendiente') as "Cuenta Contable",
                    e.description as "Detalle Contable"
                FROM raw_bank b
                LEFT JOIN expense_ledger e ON b.id::text = ANY(string_to_array(e.source_bank_id, ','))
                LEFT JOIN expense_categories c ON e.category_id = c.id
                WHERE b.bank_date BETWEEN :start AND :end
                ORDER BY b.bank_date DESC
            ''')
            with engine.connect() as conn:
                df_cartola = pd.read_sql(q_cartola, conn, params=params)
            
            if not df_cartola.empty:
                df_cartola['Fecha'] = pd.to_datetime(df_cartola['Fecha']).dt.strftime('%d/%m/%Y')
                
                # Función para pintar filas sin asignar
                def row_style(row):
                    if row['Cuenta Contable'] == 'Sin Asignar / Pendiente':
                        if row['Monto'] < 0:
                ...
                st.dataframe(
                    df_cartola.style.apply(row_style, axis=1).format({"Monto": "${:,.0f}", "Saldo": "${:,.0f}"}), 
                    use_container_width=True,
                    height=600
                )
            else:
                st.info("No hay movimientos bancarios en este periodo. Ve a 'Sync & Carga' para subir tu cartola.")

...
async def check_reports_menu():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            print("Navigating to Reportes...")
            await page.goto("https://boxmagic.cl/reportes")
            await page.wait_for_load_state("networkidle")
            await asyncio.sleep(5)
            
            print("Url:", page.url)
            await page.screenshot(path="reports_index.png", full_page=True)
            
            with open("reports_index.html", "w", encoding="utf-8") as f:
                f.write(await page.content())
                
        except Exception as e:
            print(f"Error: {e}")
            await page.screenshot(path="error_reports_index.png")
        finally:
            await browser.close()

if __name__ == "__main__":
    asyncio.run(check_reports_menu())
